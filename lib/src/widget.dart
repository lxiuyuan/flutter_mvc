...
///节省没必要build次数，可以有效多节约系统资源
///通过diff算法整理
class LocalBind extends StatefulWidget {
  //
  final List<dynamic> Function() bind;
  final Widget child;

  LocalBind({this.bind, @required this.child});

  @override
  StatefulElement createElement() {
    return MvcStatefulElement(this);
  }

  @override
  LocalBindState createState() => LocalBindState();
}

class LocalBindState extends State<LocalBind> {
  MvcAttribute mvcAttribute;
  List<dynamic> oldDiffs = [];
  ...
  void _init() {
    ...
    if (widget.bind != null) {
      oldDiffs = _listNew(widget.bind());
      mvcAttribute.listStatefulNew.add(this);
    }
  }

  @override
  void didUpdateWidget(LocalBind oldWidget) {
    if (widget.bind != null) {
      oldDiffs = _listNew(widget.bind());
      setState(() {});
    }
  ...
  void _refresh() {
    (context as MvcStatefulElement).refresh();
//    setState(() {});
  }
